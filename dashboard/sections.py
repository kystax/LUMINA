...
if hasattr(st, "dialog"):
    @st.dialog("📄 Clinical Reports & Export")
    def _open_reports_dialog(user_name: str, analysis_data: dict | None):
        ...
        st.markdown(f"#### Clinical Assessment Report for **{html.escape(user_name)}**")
        st.caption("Standard Medical Assessment & Care Plan documentation under CPT 99483 guidelines.")

        active_analysis = analysis_data or {}
        if not active_analysis:
            ...
            if recent_results:
                ...
                if isinstance(latest, (list, tuple)) and len(latest) > 4:
                    ...
                    if sess_ids:
                        active_analysis = get_analysis_by_session_id(sess_ids[0]) or {}
        ...
        st.download_button(
            "⬇️ Download Cognitive Assessment & Care Plan (PDF)",
            data=pdf_bytes,
            file_name=f"{user_name.lower().replace(' ', '_')}_clinical_assessment.pdf",
            mime="application/pdf",
            type="primary",
            use_container_width=True,
            key="dialog_pdf_download_btn",
        )
        st.markdown("<div style='height: 8px;'></div>", unsafe_allow_html=True)
        st.info("ℹ️ Generated by LUMINA Cognitive AI Research Group · Sri Lanka Technology Campus (SLTC).")

    @st.dialog("📖 LUMINA Research Methodology")
    def _open_methodology_dialog():
        st.markdown("#### LUMINA Cognitive Pattern Risk Framework")
        st.markdown("""
        LUMINA is an academic research platform integrating multi-modal digital biomarkers to analyze longitudinal cognitive and communication patterns:

        1. **Digital Footprint Intake**: Processes standardized data exports from Instagram, Facebook, Threads, TikTok, and YouTube.
        2. **NLP Linguistic Features**: Analyzes Lexical Diversity (TTR), syntactic complexity, semantic coherence via multilingual BERT (mBERT), and phrase repetition.
        3. **Social Network Dynamics (SNA)**: Quantifies withdrawal trends, posting frequencies, and interaction diversity over monthly windows.
        4. **Lancet Commission Risk Factors**: Synthesizes 14 modifiable dementia risk factors and self-reported symptoms.
        5. **Agent-Based Simulation (ABM)**: Projects 12-month cognitive trajectories under modifiable lifestyle interventions.

        > **Ethical & Research Notice**: LUMINA is intended for research-based risk screening and not for clinical diagnosis.
        """)
else:
    def _open_reports_dialog(user_name: str, analysis_data: dict | None):
        st.session_state["show_reports_modal"] = True

    def _open_methodology_dialog():
        st.session_state["show_methodology_modal"] = True
    ...
    with tab_overview:
        ...
        with r_col3:
            with st.container(border=True):
                ...
                if st.button("☁️ **Upload new data**  \n:gray[Add a new export file to analyze]", key="qa_btn_upload", use_container_width=True):
                    st.session_state["show_upload_modal"] = True
                    st.rerun()

                st.markdown("<div style='height: 2px;'></div>", unsafe_allow_html=True)

                if st.button("📄 **View reports**  \n:gray[Download reports and insights]", key="qa_btn_reports", use_container_width=True):
                    _open_reports_dialog(display_name, analysis)

                st.markdown("<div style='height: 2px;'></div>", unsafe_allow_html=True)

                if st.button("📖 **Learn more**  \n:gray[About LUMINA methodology]", key="qa_btn_learn", use_container_width=True):
                    _open_methodology_dialog()
